...
//     - Infarct remodelling in healthy, border, and infarct zone through the ischemic, and acute
...
//     infarct stage: 0=healthy, 1=ischemic, 2=acute, 3=acute-chronic, 4=chronic
...
const real cao = 1.8;
real ko = 5.0;
if((infarct_zone == 1) && ((infarct_stage == 1) || (infarct_stage == 2))) {
    ko = 8.5;
}
...
real aCaMK = 0.05;
if(((infarct_zone == 1) || (infarct_zone == 2)) && ((infarct_stage == 2) || (infarct_stage == 3))) {
    aCaMK = aCaMK * 1.5;
...
//Current
real GNa = 11.7802;
if((infarct_zone == 1) && (infarct_stage == 1)) {
    GNa = GNa * 0.75;
} else if(((infarct_zone == 1) || (infarct_zone == 2)) && (infarct_stage == 2)) {
    GNa = GNa * 0.575;
} else if(((infarct_zone == 1) || (infarct_zone == 2)) && (infarct_stage == 3)) {
    GNa = GNa * 0.4;
} else if((infarct_zone == 1) && (infarct_stage == 4)) {
    GNa = GNa * 0.4;
} else if(((infarct_zone == 0) || (infarct_zone == 2)) && (infarct_stage == 4)) {
    GNa = GNa * 0.43;
}
...
if((infarct_zone == 2) && (infarct_stage == 4)) {
    GNaL = GNaL * 1.275;
} else if((infarct_zone == 0) && (infarct_stage == 4)) {
    GNaL = GNaL * 1.413;
}
...
real dnca_i = anca_i*k2n-nca_i*km2n;
rDY[30] = nca_i + dt * dnca_i;
//SS driving force
...
if((infarct_zone == 1) && (infarct_stage == 1)) {
    ...
    PCa = PCa * 0.64;
} else if((infarct_zone == 2) && (infarct_stage == 4)) {
    PCa = PCa * 0.7;
}
real PCap = 1.1*PCa;
...
real dikr_c0 = ikr_c1 * beta - ikr_c0 * alpha; //delta for c0
rDY[37] = ikr_c0 + dt * dikr_c0;
real dikr_c1 = ikr_c0 * alpha + ikr_c2 * beta1 - ikr_c1 * (beta + alpha1); //c1
rDY[38] = ikr_c1 + dt * dikr_c1;
real dikr_c2 = ikr_c1 * alpha1 + ikr_o * beta2 + ikr_i * betaItoC2 - ikr_c2 * (beta1 + alpha2 + alphac2ToI); //subtraction is into c2, to o, to i.
rDY[39] = ikr_c2 + dt * dikr_c2;
real dikr_o = ikr_c2 * alpha2 + ikr_i * betai - ikr_o * (beta2 + alphai);
rDY[40] = ikr_o + dt * dikr_o;
real dikr_i = ikr_c2 * alphac2ToI + ikr_o * alphai - ikr_i * (betaItoC2 + betai);
rDY[41] = ikr_i + dt * dikr_i;
//Current
real GKr = 0.0321 * sqrt(ko/5); //1st element compensates for change to ko (sqrt(5/5.4)* 0.0362)
if(layer == 3) {
    GKr = GKr*1.3;
} else if(layer == 2) {
    GKr=GKr*0.8;
}
if((infarct_zone == 1) && ((infarct_stage == 2) || (infarct_stage == 3))) {
    GKr = GKr * 0.375;
} else if((infarct_zone == 2) && ((infarct_stage == 2) || (infarct_stage == 3))) {
    GKr = GKr * 0.7;
} else if((infarct_zone == 1) && (infarct_stage == 4)) {
    GKr = GKr * 0.7;
} else if((infarct_zone == 2) && (infarct_stage == 4)) {
...
//Current
real GKs = 0.0011 * (pow(0.2, (2*apicobasal - 1))); //Apicobasal gradient normalised to produce scaling factor between 0.2 and 5.0
if(layer == 3) {
    GKs = GKs*1.4;
}
...
real GClCa = 0.2843;
if(((infarct_zone == 0) || (infarct_zone == 2)) && (infarct_stage == 4)) {
    GClCa = GClCa * 1.25;
}
real GClB = 1.98e-3;
real KdClCa = 0.1;
real I_CaCl_junc = Fjunc*GClCa/(1+KdClCa/cass)*(v-eclss);
real I_CaCl_sl = Fsl*GClCa/(1+KdClCa/cai)*(v-ecl);
real I_CaCl = I_CaCl_junc+I_CaCl_sl;
real I_Clbk = GClB*(v-ecl);
//IKCa
//As in Zhou and Wang et al., 2022
real GKCa = 0.003;
if((infarct_stage == 4) && (infarct_zone != 1)) {
    GKCa = GKCa * 2.0;
}
real IKCan = 3.5;
...
real K_o_n = 5.4;
real fkatp = 0.0;
if((infarct_stage == 1) && (infarct_zone == 1)) {
    fkatp = 0.07;
}
real gkatp = 4.4;
...
real Jupnp = 0.005425*cai/(cai+0.00092);
real Jupp = 2.75*0.005425*cai/(cai+0.00092-0.00017);
if(layer == 3) {
    Jupnp = Jupnp*1.3;
    Jupp = Jupp*1.3;
}
...
if(((infarct_zone == 0) || (infarct_zone == 2)) && (infarct_stage == 4)) {
    Jup_scaling = Jup_scaling * 0.4;
}
//Fluxes
real Jleak = 0.0048825*cansr/15.0;
real Jup = Jup_scaling * (1.0-fJupp)*Jupnp+fJupp*Jupp-Jleak;
...
// Itot
real Itot = INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+INaCa_i+INaCa_ss+INaK+INab+IKb+IpCa+ICab+I_CaCl+I_Clbk+IKCa+IKatp;
real dv = -(Itot + Istim);
rDY[0] = v + dt * dv;
...
rDY[2] = nass + dt * dnass;
real dki = -(ICaK_i+Ito+IKr+IKs+IK1+IKb+Istim-2.0*INaK)*Acap/(F*vmyo)+JdiffK*vss/vmyo;
rDY[3] = ki + dt * dki;
...
rDY[6] = cass + dt * dcass;
real dcli = - (I_Clbk + I_CaCl_sl)*Acap/(-1*F*vmyo)+JdiffCl*vss/vmyo;
rDY[43] = cli + dt * dcli;
real dclss = - I_CaCl_junc*Acap/(-1*F*vss)-JdiffCl;
rDY[44] = clss + dt * dclss;
real dcansr = Jup-Jtr*vjsr/vnsr;
rDY[7] = cansr + dt * dcansr;
real Bcajsr = 1.0/(1.0+csqnmax*kmcsqn/pow((kmcsqn+cajsr),2));
real dcajsr = Bcajsr*(Jtr-Jrel);
rDY[8] = cajsr + dt * dcajsr;
